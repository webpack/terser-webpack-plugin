...
```ts
type minifyEmbedded = boolean;
```

Default: `true`

Minify source written in one language that reaches the bundle inside another,
which no asset carries and an asset-level minimizer therefore never sees
(requires webpack `>= 5.110.0`; older versions ignore this option).
